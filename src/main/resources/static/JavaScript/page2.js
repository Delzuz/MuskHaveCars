...
const grabSelect = document.querySelector(".grabSelect")
function printSorting() {
    const grabSelectValue = grabSelect.value
    printCars(grabSelectValue)

}

printCars()
async function printCars(grabSelectValue) {
    let bilar = await getAllCars()
    let carQuant = await getAllQuantity()
    if (grabSelectValue === "range") {
        bilar = await getAllCars()
        bilar.sort((a,b) => b.range - a.range)
    }
    if (grabSelectValue === "reset") {
        bilar = await getAllCars()
        
    }
    if (grabSelectValue === "price") {
        bilar = await getAllCars()
        bilar.sort((a, b) => a.carSegment.price - b.carSegment.price)
    }
